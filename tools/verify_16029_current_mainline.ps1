...
Invoke-CheckInDirectory -Name "verified_v43_door_panel_rebuild_gate_contract" -WorkingDirectory $Root -Command "`$worker = Get-Content -LiteralPath 'tools\generate_review_solidworks_full_assembly.ps1' -Raw -Encoding UTF8; `$repair = Get-Content -LiteralPath 'workers\solidworks_tools\RepairVerifiedV43DoorPanelFeature.cs' -Raw -Encoding UTF8; `$restore = Get-Content -LiteralPath 'workers\solidworks_tools\RestoreDoorLockTongues.cs' -Raw -Encoding UTF8; `$remove = Get-Content -LiteralPath 'workers\solidworks_tools\RemoveAssemblyComponentsByPattern.cs' -Raw -Encoding UTF8; `$inspect = Get-Content -LiteralPath 'workers\solidworks_tools\InspectAssemblyComponents.cs' -Raw -Encoding UTF8; if (`$worker -notmatch 'RepairVerifiedV43DoorPanelFeature.exe' -or `$worker -notmatch 'solidworks_2020_verified_v43_door_panel_feature_repair.json' -or `$worker -notmatch 'RequireRebuild' -or `$repair -notmatch 'known verified-v43 generated door-panel copy' -or `$repair -notmatch 'GeometryUnchanged' -or `$restore -notmatch '!item.Rebuilt' -or `$remove -notmatch 'result.Opened && result.Rebuilt' -or `$inspect -notmatch '--require-rebuild' -or `$inspect -notmatch 'assembly rebuild failed') { throw 'the verified v43 generated-copy repair and final SolidWorks rebuild gates are not wired through the generator' }"
...
Invoke-CheckInDirectory -Name "single_door_mechanical_lock_tongue_contract" -WorkingDirectory $Root -Command "`$single = Get-Content -LiteralPath 'tools\generate_review_solidworks_single_door.ps1' -Raw -Encoding UTF8; `$builder = Get-Content -LiteralPath 'workers\solidworks_tools\BuildOrdinaryDoorModule.cs' -Raw -Encoding UTF8; if (`$single -notmatch 'mechanical-lock-tongue' -or `$single -notmatch 'lockTonguePart' -or `$single -notmatch 'Copy-Item -LiteralPath' -or `$single -notmatch 'lockHook' -or `$builder -notmatch 'MechanicalLockTongue' -or `$builder -notmatch 'lock_tongue') { throw 'single-door generator must restore a mechanical lock tongue from the v43/gold source geometry instead of dropping the lock-side tongue' }"
...
Invoke-CheckInDirectory -Name "solidworks_background_session_boundary_contract" -WorkingDirectory $Root -Command "`$worker = Get-Content -LiteralPath 'tools\generate_review_solidworks_full_assembly.ps1' -Raw -Encoding UTF8; `$builder = Get-Content -LiteralPath 'workers\solidworks_tools\BuildPlacedComponentsModule.cs' -Raw -Encoding UTF8; `$importer = Get-Content -LiteralPath 'workers\solidworks_tools\ImportStepSaveNative.cs' -Raw -Encoding UTF8; `$inspector = Get-Content -LiteralPath 'workers\solidworks_tools\InspectAssemblyComponents.cs' -Raw -Encoding UTF8; `$pack = Get-Content -LiteralPath 'workers\solidworks_tools\PackAndGoAssembly.cs' -Raw -Encoding UTF8; `$queue = Get-Content -LiteralPath 'tools\process_16029_review_generation_queue.mjs' -Raw -Encoding UTF8; if (`$worker -notmatch 'Wait-NoRunningSolidWorks' -or `$worker -notmatch 'Invoke-IsolatedPlacedAssemblyBuild' -or `$worker -notmatch 'FixedModuleBuildTimeoutSeconds' -or `$worker -notmatch 'Pack-and-Go post-processing session cleanup' -or `$worker -notmatch '--exit-session' -or `$builder -notmatch '--isolated-session' -or `$builder -notmatch 'isolated session requires no running SolidWorks process' -or `$importer -notmatch '--exit-session' -or `$inspector -notmatch 'solidworks_process_id' -or `$pack -notmatch 'solidworks_process_id' -or `$queue -notmatch 'Keep SolidWorks closed') { throw 'background SolidWorks generation must reject active engineer sessions, close the STEP import session, isolate assembly builds, clean inspection and Pack-and-Go post-processing sessions by recorded PID, and explain the session boundary in the task card' }"
