...
$source = Join-Path $toolDir 'RenameAssemblyComponents.cs'
$outputDir = Join-Path $toolDir 'bin'
$output = Join-Path $outputDir 'RenameAssemblyComponents.exe'
...
if ($LASTEXITCODE -ne 0) {
  throw "RenameAssemblyComponents compilation failed with exit code $LASTEXITCODE"
}
