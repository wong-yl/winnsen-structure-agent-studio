...
$source = Join-Path $toolDir 'BuildCenteredBackSeamAssembly.cs'
$outputDir = Join-Path $toolDir 'bin'
$output = Join-Path $outputDir 'BuildCenteredBackSeamAssembly.exe'
...
if ($LASTEXITCODE -ne 0) {
  throw "BuildCenteredBackSeamAssembly compilation failed with exit code $LASTEXITCODE"
}
