...
$source = Join-Path $toolDir 'BuildPlacedComponentsModule.cs'
$outputDir = Join-Path $toolDir 'bin'
$output = Join-Path $outputDir 'BuildPlacedComponentsModule.exe'
...
if ($LASTEXITCODE -ne 0) {
  throw "BuildPlacedComponentsModule compilation failed with exit code $LASTEXITCODE"
}
