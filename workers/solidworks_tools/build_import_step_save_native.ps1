...
$source = Join-Path $toolDir 'ImportStepSaveNative.cs'
$outputDir = Join-Path $toolDir 'bin'
$output = Join-Path $outputDir 'ImportStepSaveNative.exe'
...
if ($LASTEXITCODE -ne 0) {
  throw "ImportStepSaveNative compilation failed with exit code $LASTEXITCODE"
}
