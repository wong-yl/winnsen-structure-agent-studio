...
$source = Join-Path $toolDir 'RestoreDoorLockTongues.cs'
$outputDir = Join-Path $toolDir 'bin'
$output = Join-Path $outputDir 'RestoreDoorLockTongues.exe'
...
if ($LASTEXITCODE -ne 0) {
  throw "RestoreDoorLockTongues compilation failed with exit code $LASTEXITCODE"
}
