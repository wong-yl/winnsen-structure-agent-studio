...
$source = Join-Path $toolDir 'PackAndGoAssembly.cs'
$outputDir = Join-Path $toolDir 'bin'
$output = Join-Path $outputDir 'PackAndGoAssembly.exe'
...
if ($LASTEXITCODE -ne 0) {
  throw "PackAndGoAssembly compilation failed with exit code $LASTEXITCODE"
}
