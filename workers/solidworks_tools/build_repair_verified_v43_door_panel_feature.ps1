...
$source = Join-Path $toolDir 'RepairVerifiedV43DoorPanelFeature.cs'
$outputDir = Join-Path $toolDir 'bin'
$output = Join-Path $outputDir 'RepairVerifiedV43DoorPanelFeature.exe'
...
if ($LASTEXITCODE -ne 0) {
  throw "RepairVerifiedV43DoorPanelFeature compilation failed with exit code $LASTEXITCODE"
}
