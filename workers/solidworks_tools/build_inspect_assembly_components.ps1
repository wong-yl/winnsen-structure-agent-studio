...
$source = Join-Path $toolDir 'InspectAssemblyComponents.cs'
$outputDir = Join-Path $toolDir 'bin'
$output = Join-Path $outputDir 'InspectAssemblyComponents.exe'
...
if ($LASTEXITCODE -ne 0) {
  throw "InspectAssemblyComponents compilation failed with exit code $LASTEXITCODE"
}
