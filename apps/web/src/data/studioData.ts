...
export const projects: Project[] = [
  ...
  },
  {
    ...
    status: '10/12/14 门原生 SolidWorks 增强矩阵样机已验证；FreeCAD 规则结果保留为验证线',
    statusTone: 'good',
    sourceRoot: 'C:\\Users\\Administrator\\Desktop\\参数化模板素材\\16029 寄存柜(标准组合式 1917×1000×550)',
    modelStatus: '10/12/14 门增强矩阵样机 v2 可打开，均已加入 9 个固定模块',
    progress: 88,
    capability: '10/12/14 门 SolidWorks enriched native reference + FreeCAD rule-driven 参考模型；规则证据含门高、门距、层板/门框、锁/铰链/五金重排表',
    risk: '当前仍是工程参考模型，不是正式 SolidWorks 工程图/DXF/BOM；后续需补后门、线束、Pack-and-Go 和出图流程',
    ...
    notes: [
      ...
      'SolidWorks 入口已改为 10/12/14 门优先打开并另存增强矩阵样机；下一步补后侧/电气模块与 Pack-and-Go。',
    ],
  },
  {
...
export const pipelineRows: PipelineRow[] = [
  ...
  },
  {
    ...
    next: '把后门、维护门、电气件、Pack-and-Go 和工程图流程接到 10/12/14 增强样机',
...
export const ruleLearningAxes: RuleLearningAxis[] = [
  {
    ...
    currentState: '16029 已有 10/12/14 门原生 SolidWorks 增强矩阵样机 v2；STEP 角色绑定 177、重复角色组 34、阵列种子 3。transform 自动重排仍未扩展到所有后侧/电气模块。',
    nextAction: '先做 10/12/14 Pack-and-Go，再把后门、维护门、电气件、锁/铰链规则补进增强样机生成链。',
    maturity: 'step_bbox_measured',
...
export const capabilities: Capability[] = [
  ...
  },
  {
    ...
    variants: '1000×1917×550 外型下 10/12/14 门原生 SolidWorks 增强样机；FreeCAD 作为规则验证通道',
    status: 'generatable',
    maturity: 'engineering_reference',
    ...
    evidence: ['DXF', 'STEP', 'SolidWorks', 'FreeCAD', 'BOM'],
    limitation:
      'SolidWorks 是当前工程复核工具；逐零件直装路线因装配基准/transform 视觉错乱已降级。当前主交接物为 10/12/14 门原生整柜增强矩阵样机 v2 与 FreeCAD 规则参考模型，不作为正式生产图纸；任意门宽/门高变化仍需补齐门框、锁位、铰链和 BOM/DXF 公式闭环。',
  },
  {
...
export const reviewItems: ReviewItem[] = [
  {
    ...
    priority: 'P1',
    severity: '严重',
    issue: '10/12/14 门增强样机已验证，但还缺后门、维护门、电气件以及 transform/mate 级别锁、铰链自动重排器',
  ...
  },
  {
    ...
    priority: 'P1',
    severity: '严重',
    issue: '10/12/14 门增强样机未做 Pack-and-Go 封包，移动 SLDASM 时仍依赖源文件目录',
    nextAction: '给 10/12/14 门增强样机增加 Pack-and-Go 或明确复制依赖清单，保证结构工程师在独立目录打开不丢引用。',
