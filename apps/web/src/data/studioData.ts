...
export const projects: Project[] = [
  ...
  },
  {
    ...
    status: '12 门原生 SolidWorks 增强矩阵样机已验证；10/14 门骨架与 FreeCAD 规则结果保留为验证线',
    statusTone: 'good',
    sourceRoot: 'C:\\Users\\Administrator\\Desktop\\参数化模板素材\\16029 寄存柜(标准组合式 1917×1000×550)',
    modelStatus: '12 门增强矩阵样机 v2 可打开，已加入 9 个固定模块；10/14 门原生骨架 v2 保持可复核',
    progress: 88,
    capability: '12 门 SolidWorks enriched native reference + 10/14 门 native skeleton + FreeCAD rule-driven 参考模型；规则证据含门高、门距、层板/门框、锁/铰链/五金重排表',
    risk: '当前仍是工程参考模型，不是正式 SolidWorks 工程图/DXF/BOM；后续需把 12 门固定模块矩阵规则推广到 10/14 门，并补后门、线束、Pack-and-Go 和出图流程',
    updatedAt: '2026-05-21 14:40',
    stats: [
    ...
    notes: [
      '16029 12 门增强矩阵样机已写入 workers/generated_models/SW-NATIVE-16029-CABINET-ENRICHED-12DOOR-20260521；9 个固定模块 bbox 匹配通过。',
      '16029 10/14 门原生 SolidWorks 骨架仍保留在 workers/generated_models/SW-NATIVE-16029-CABINET-SKELETON-SERIES-20260521。',
      ...
      'SolidWorks 入口已改为 12 门优先打开并另存增强矩阵样机；下一步把固定模块矩阵规则推广到 10/14 门。',
    ],
  },
  {
...
export const capabilities: Capability[] = [
  ...
  },
  {
    ...
    variants: '1000×1917×550 外型下 12 门原生 SolidWorks 增强样机，10/14 门原生骨架；FreeCAD 作为规则验证通道',
    status: 'generatable',
    maturity: 'engineering_reference',
    ...
    evidence: ['DXF', 'STEP', 'SolidWorks', 'FreeCAD', 'BOM'],
    limitation:
      'SolidWorks 是当前工程复核工具；逐零件直装路线因装配基准/transform 视觉错乱已降级。当前主交接物为 12 门原生整柜增强矩阵样机 v2、10/14 门骨架 v2 与 FreeCAD 规则参考模型，不作为正式生产图纸；任意门宽/门高变化仍需补齐门框、锁位、铰链和 BOM/DXF 公式闭环。',
  },
  {
