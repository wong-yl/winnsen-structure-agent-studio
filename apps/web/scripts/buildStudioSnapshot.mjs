...
async function sourceInfo(filePath) {
  try {
    ...
    return { path: filePath, updatedAt: stat.mtime.toISOString(), bytes: stat.size }
  } catch {
    return { path: filePath, updatedAt: null, bytes: 0 }
  }
}

const [intakeStatus, outdoorStatus, strictRowsText, outdoorRowsText, outdoorSingleDoorAudit] = await Promise.all([
  readTextSafe(sources.intakeStatus),
  readTextSafe(sources.outdoorStatus),
  readTextSafe(sources.strictQueueCsv),
  readTextSafe(sources.outdoorGateCsv),
  readTextSafe(sources.outdoorSingleDoorAudit),
])
